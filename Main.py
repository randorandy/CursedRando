...
def assumed_fill(game: Game) -> tuple[bool]:
    ...
    while fill_algorithm.count_items_remaining():
        ...
        if fill_algorithm.count_items_remaining() == 0:
            ...
            completable = True
            print(game.all_locations["Morph Ball"]["item"][0])
            if completable:
...
def write_rom(game: Game, romWriter: Optional[RomWriter] = None) -> str:
    ...
    rom_name = f"Cursed{game.seed}.sfc"
    rom1_path = f"roms/{rom_name}"
    rom_clean_path = "roms/Cursed.sfc"
    ...
    # accessible morph patch
    romWriter.writeBytes(0x79e11, b"\x07")

    # escape morph patch
    romWriter.writeBytes(0x7bb86, b"\x30\x08\x00\x00")    
    romWriter.writeBytes(0x78086, b"\x84")
    ...
    '''
    patches_list = ["Patches/shortmessageboxes_ver3.ips"]
